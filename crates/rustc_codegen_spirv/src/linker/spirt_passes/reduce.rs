                    ...
                    match &node_def.kind {
                        ...
                        NodeKind::Loop { repeat_condition } => {
                            ...
                            for i in 0..loop_state_count {
                                ...
                                let loop_output = func_def_body.at(node).def().outputs[i];
                                ...
                                if body_output == Value::Var(body_input) {
                                    ...
                                    var_replacer
                                        .maybe_add_replacement_with_if_new(loop_output, || {
                                            Some(initial_input)
                                        });
                                }

                                // FIXME(eddyb) consider replacing `loop_output`
                                // with `body_output`, when the latter is a value
                            ...
                            if let Some(tail_node) =
                                ...
                                // FIXME(eddyb) this entire thing might be broken??
                                // (there's no check for `NodeKind::Select`...)
                                && false
                            {
