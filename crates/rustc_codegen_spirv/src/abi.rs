        ...
        match self.map.borrow_mut().entry(pointee) {
            ...
            Entry::Occupied(mut entry) => match *entry.get() {
                ...
                CycleDetectionState::InProgress => {
                    let id = SpirvType::Pointer { pointee: None }.def(span, cx);
            ...
            Abi::ScalarPair(a, b) => {
                ...
                if let Some((i, field)) = sole_non_zst_field {
                    ...
                    if self.fields.offset(i) == Size::ZERO
                        ...
                        && field.abi.eq_up_to_validity(&self.abi)
                    {
    ...
    match scalar.primitive() {
        ...
        Primitive::Pointer(_) => {
            let Ok(Some(pointee_ty)) = dig_scalar_pointee(cx, ty, offset) else {
                return SpirvType::Pointer { pointee: None }.def(span, cx);
            };
        ...
        IntrinsicType::RuntimeArray => {
            ...
            if let Some(elem_ty) = args.types().next() {
                ...
                Ok(SpirvType::RuntimeArray {
                    element: elem.spirv_type(span, cx),
                    stride: elem.size,
                }
                .def(span, cx))
            } else {
